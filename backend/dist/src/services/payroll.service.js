...
let PayrollService = PayrollService_1 = class PayrollService {
    ...
    async processPayroll(payload, initiator) {
        try {
            ...
            if (workflowType === 'leave-allowance') {
                const leaveApprovalLink = await this.resolveLeaveAllowanceNoticeLink(approvalId);
                await this.notifyLeaveAllowanceAssignees(reviewerIds, 'Leave allowance batch is awaiting your review.', leaveApprovalLink);
                await this.notifyLeaveAllowanceAssignees(auditViewerIds, 'Leave allowance batch is ready for audit/finance viewing.', leaveApprovalLink);
            }
            else {
                await this.notifyStageAssignees(reviewerIds, approvalId, entityName, monthLabel, 'REVIEWER');
                await this.notifyAuditViewers(auditViewerIds, approvalId, entityName, monthLabel);
            }
            return {
                status: 200,
    ...
    async approveLeaveAllowanceApproval(user, id, comment) {
        ...
        if (approval.status === 'PENDING_REVIEW') {
            const approvalLink = this.buildLeaveApprovalNoticeLink(approval);
            ...
            await this.notifyLeaveAllowanceAssignees(approval.approverIds, 'Leave allowance batch is awaiting your approval.', approvalLink);
        ...
        if (approval.status === 'PENDING_APPROVAL') {
            ...
            await approval.save();
            await this.syncPayrollApprovalFromLeave(approval);
            await this.notifyLeaveAllowanceAssignees(approval.postingIds, 'Leave allowance batch is awaiting your posting.', approvalLink);
    ...
    async rejectLeaveAllowanceApproval(user, id, reason) {
        ...
        await approval.save();
        await this.syncPayrollApprovalFromLeave(approval);
        await this.notifyLeaveAllowanceRequester(approval, 'Your leave allowance batch has been rejected.', this.buildLeaveApprovalNoticeLink(approval));
    ...
    async markLeaveAllowancePostingComplete(user, id) {
        ...
        await approval.save();
        await this.syncPayrollApprovalFromLeave(approval);
        await this.notifyLeaveAllowanceRequester(approval, 'Your leave allowance batch has been posted.', this.buildLeaveApprovalNoticeLink(approval));
